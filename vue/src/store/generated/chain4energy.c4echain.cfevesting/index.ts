...
import { VestingPoolReservation } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
import { EventNewVestingAccount } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
import { EventNewVestingPool } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
import { EventNewVestingPeriodFromVestingPool } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
import { EventWithdrawAvailable } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
import { EventVestingSplit } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfevesting/types"
...
export { AccountVestingPools, VestingPool, VestingPoolReservation, EventNewVestingAccount, EventNewVestingPool, EventNewVestingPeriodFromVestingPool, EventWithdrawAvailable, EventVestingSplit, GenesisVestingType, Params, VestingPoolInfo, VestingAccountTrace, ContinuousVestingPeriod, PeriodicContinuousVestingAccount, VestingTypes, VestingType };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						...
						VestingPoolReservation: getStructure(VestingPoolReservation.fromPartial({})),
						EventNewVestingAccount: getStructure(EventNewVestingAccount.fromPartial({})),
						EventNewVestingPool: getStructure(EventNewVestingPool.fromPartial({})),
						EventNewVestingPeriodFromVestingPool: getStructure(EventNewVestingPeriodFromVestingPool.fromPartial({})),
						EventWithdrawAvailable: getStructure(EventWithdrawAvailable.fromPartial({})),
						EventVestingSplit: getStructure(EventVestingSplit.fromPartial({})),
...
export default {
	...
	actions: {
		...
		async sendMsgUpdateDenomParam({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.Chain4EnergyC4EchainCfevesting.tx.sendMsgUpdateDenomParam({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
