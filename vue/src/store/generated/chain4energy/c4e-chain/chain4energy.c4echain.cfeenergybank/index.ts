...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: chain4energy.c4echain.cfeenergybank initialized!')
		...
		async QueryEnergyTokenUserAddress({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryEnergyTokenUserAddress( key.userAddress, query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryEnergyTokenUserAddress( key.userAddress, {...query, 'pagination.key':(<any> value).pagination.next_key})).data
	...
	gas: "200000" }, memo})
		...
		async sendMsgTransferTokensOptimally({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const txClient=await initTxClient(rootGetters)
				const msg = await txClient.msgTransferTokensOptimally(value)
				const result = await txClient.signAndBroadcast([msg], {fee: { amount: fee, 
	gas: "200000" }, memo})
				return result
			} catch (e) {
				if (e == MissingWalletError) {
					...
					throw new Error('TxClient:MsgTransferTokensOptimally:Send Could not broadcast Tx: '+ e.message)
				}
			}
		},
