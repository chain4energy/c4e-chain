...
import { EventDistribution } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfedistributor/types"
import { EventDistributionBurn } from "chain4energy-c4e-chain-client-ts/chain4energy.c4echain.cfedistributor/types"
...
export { EventDistribution, EventDistributionBurn, Params, State, SubDistributor, Destinations, DestinationShare, Account };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						EventDistribution: getStructure(EventDistribution.fromPartial({})),
						EventDistributionBurn: getStructure(EventDistributionBurn.fromPartial({})),
...
export default {
	...
	actions: {
		...
		async sendMsgUpdateSubDistributorParam({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.Chain4EnergyC4EchainCfedistributor.tx.sendMsgUpdateSubDistributorParam({ value, fee: fullFee, memo })
				return result
			} catch (e) {
		...
		async sendMsgUpdateParams({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.Chain4EnergyC4EchainCfedistributor.tx.sendMsgUpdateParams({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgUpdateSubDistributorBurnShareParam({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.Chain4EnergyC4EchainCfedistributor.tx.sendMsgUpdateSubDistributorBurnShareParam({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgUpdateSubDistributorDestinationShareParam({ rootGetters }, { value, fee = {amount: [], gas: "200000"}, memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const fullFee = Array.isArray(fee)  ? {amount: fee, gas: "200000"} :fee;
				const result = await client.Chain4EnergyC4EchainCfedistributor.tx.sendMsgUpdateSubDistributorDestinationShareParam({ value, fee: fullFee, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
