...
func TestMinterWithDistributor2(t *testing.T) {
	tests := []struct {
		name        string
		timeInYears int
		want        testResult
...
func aaaa(timeInYear int) testResult {
	...
	ctx.Logger().Error("ASDASDASDA")

	oneYearDuration := 365 * 24 * time.Hour

	ctx = ctx.WithBlockHeight(int64(99)).WithBlockTime(ctx.BlockTime().Add(oneYearDuration))
	app.BeginBlocker(ctx, abci.RequestBeginBlock{})
	app.EndBlocker(ctx, abci.RequestEndBlock{})
	plan := upgradetypes.Plan{Name: "v2", Height: 100}
	app.UpgradeKeeper.ScheduleUpgrade(ctx, plan)
	ctx = ctx.WithBlockHeight(int64(100)).WithBlockTime(ctx.BlockTime().Add(oneYearDuration))
	app.BeginBlocker(ctx, abci.RequestBeginBlock{})
	app.EndBlocker(ctx, abci.RequestEndBlock{})
