...
func (s *EvSetupSuite) TestEvLogic() {
	...
	node.RemoveEnergyTransferError(stringTransferId, driverWalletName, fmt.Sprintf("energy transfer status is not PAID or CANCELLED"))

	node.EnergyTransferStarted(stringTransferId, stringOfferId, "", driverWalletName)
	node.CancelEnergyTransferdError(stringTransferId, "", "", driverWalletName, "energy transfer wrong status")
