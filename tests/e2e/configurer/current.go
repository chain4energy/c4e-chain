...
func (cb *CurrentBranchConfigurer) ConfigureChain(chainConfig *chain.Config) error {
	...
	initializedChain, err := initialization.InitChain(chainConfig.Id, tmpDir, chainConfig.ValidatorInitConfigs, time.Duration(chainConfig.VotingPeriod*1000000000), time.Duration(chainConfig.ExpeditedVotingPeriod*1000000000), 0, nil)
	if err != nil {
		return err
	}
