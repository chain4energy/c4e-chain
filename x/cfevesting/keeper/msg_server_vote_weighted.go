...
func (k msgServer) VoteWeighted(goCtx context.Context, msg *types.MsgVoteWeighted) (*types.MsgVoteWeightedResponse, error) {
	...
	voteMsg := govtypes.MsgVoteWeighted{ProposalId: msg.ProposalId,
		Voter: accVestings.DelegableAddress, Options: msg.Options}
	_, err = k.govMsgServer.VoteWeighted(goCtx, &voteMsg)
	if err != nil {
		return nil, err
	}
