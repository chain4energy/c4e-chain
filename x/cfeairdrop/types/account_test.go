...
func TestSinglePeriod(t *testing.T) {
	...
	require.True(t, acc.GetVestedCoins(checkTime).IsEqual(vested))
	require.True(t, acc.GetVestingCoins(checkTime).IsEqual(acc.OriginalVesting.Sub(vested)))
	require.True(t, acc.LockedCoins(checkTime).IsEqual(sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(2000000)))))
...
func TestMultiplePeriods(t *testing.T) {
	...
	periods := types.ContinuousVestingPeriods{
		...
		{
			Amount:    sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(100000000))),
			StartTime: p2StartTime.Unix(),
			EndTime:   p2EndTime.Unix(),
		},
		{
			Amount:    sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(1000000000))),
...
func createCorrectAirdropAccout() *types.AirdropVestingAccount {
	periods := types.ContinuousVestingPeriods{
		{
		...
		{
			Amount:    sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(800))),
			StartTime: time.Now().Add(-24 * 300 * time.Hour).Unix(),
			EndTime:   time.Now().Add(24 * 150 * time.Hour).Unix(),
		},
		{
			Amount:    sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(900000))),
			StartTime: time.Now().Add(-24 * 32 * time.Hour).Unix(),
			EndTime:   time.Now().Add(24 * 400 * time.Hour).Unix(),
		},
		{
			Amount:    sdk.NewCoins(sdk.NewCoin(commontestutils.DefaultTestDenom, sdk.NewInt(20000))),
...
func createAirdropAccout(originalVesting sdk.Coins, startTime int64, endTime int64, periods types.ContinuousVestingPeriods) *types.AirdropVestingAccount {
	return types.NewAirdropVestingAccount(&authtypes.BaseAccount{}, originalVesting, startTime, endTime, periods)
}

type AirdropAccountTc struct {
	desc    string
	account *types.AirdropVestingAccount
	valid   bool
	message string
}
