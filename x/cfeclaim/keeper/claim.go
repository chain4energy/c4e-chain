...
func (k Keeper) validateAdditionalAddressToClaim(ctx sdk.Context, additionalAddress string) error {
	...
	if err != nil {
		k.Logger(ctx).Error("add mission to claim campaign claimer parsing error", "additionalAddress", additionalAddress, "error", err.Error())
		return sdkerrors.Wrap(c4eerrors.ErrParsing, sdkerrors.Wrapf(err, "add mission to claim campaign - additionalAddress parsing error: %s", additionalAddress).Error())
	}
...
func (k Keeper) calculateAndSendInitialClaimFreeAmount(ctx sdk.Context, campaignId uint64, userEntry *types.UserEntry, claimableAmount sdk.Coins, initialClaimFreeAmount sdk.Int) (sdk.Coins, error) {
	...
	if k.bankKeeper.BlockedAddr(userMainAddress) {
		k.Logger(ctx).Error("send to claim account account is not allowed to receive funds error", "userMainAddress", userMainAddress)
		return nil, sdkerrors.Wrapf(sdkerrors.ErrUnauthorized, "send to claim account - account address: %s is not allowed to receive funds error", userMainAddress)
	}
	...
	for _, claimableAmountCoin := range claimableAmount {
		if claimableAmountCoin.Sub(sdk.NewCoin(claimableAmountCoin.Denom, initialClaimFreeAmount)).IsNegative() {
			k.Logger(ctx).Error("send to claim account wrong send coins amount. Amount < 1 token (1000000)", "amount", claimableAmountCoin.Amount, "denom", claimableAmountCoin.Denom)
			return nil, sdkerrors.Wrapf(c4eerrors.ErrSendCoins, "send to claim account  wrong send coins amount. %s < 1 token (1000000 %s)", claimableAmountCoin.String(), claimableAmountCoin.Denom)
		}
	...
	if err := k.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, userMainAddress, freeVestingAmount); err != nil {
		k.Logger(ctx).Debug("send to claim account send coins to vesting account error", "toAddress", userMainAddress,
